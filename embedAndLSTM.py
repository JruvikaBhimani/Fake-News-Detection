...
for headline, content, label in zip(data.Headline, data.Body, data.Label):
    if isinstance(headline, str) and isinstance(content, str) and (label == 0 or label == 1):
        docs.append(content)
        labels.append(label)
...
maxlen = max(num_sent) #max length of an element in docs list
print("maxlen: ", maxlen)
chars = set(txt)
global len_chars
len_chars = len(chars)
